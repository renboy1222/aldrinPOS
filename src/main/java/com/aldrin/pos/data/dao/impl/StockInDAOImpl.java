...
public class StockInDAOImpl extends DBConnection implements StockInDAO {
    ...
    public void addStockIn(StockIn stockIn) {
        try {
            getDBConn();
            java.sql.PreparedStatement ps = getCon().prepareStatement("INSERT INTO ALDRIN.STOCK_IN (ID,USER_ID,TRANSPORTER_ID) VALUES  (?,?,?) ");
    ...
    public void updateStockIn(StockIn stockIn) {
        try {
            getDBConn();
            java.sql.PreparedStatement ps = getCon().prepareStatement("UPDATE ALDRIN.STOCK_IN SET USER_ID =?, TRANSPORTER_ID =? WHERE STOCK_IN.ID = ?");
    ...
    public ArrayList<StockIn> selectStockIn() {
        ...
        try {
            String query = "SELECT \n"
                    + "    ALDRIN.STOCK_IN.ID,\n"
                    + "    ALDRIN.STOCK_IN.CREATEDAT, \n"
                    + "    ALDRIN.STOCK_IN.TRANSPORTER_ID, \n"
                    + "    ALDRIN.STOCK_IN.USER_ID, \n"
                    + "    ALDRIN.USER_ACCOUNT.FIRSTNAME AS UFIRSTNAME, \n"
                    + "    ALDRIN.USER_ACCOUNT.SURNAME AS USURNAME, \n"
                    + "    ALDRIN.TRANSPORTER.FIRSTNAME AS TFIRSTNAME, \n"
                    + "    ALDRIN.TRANSPORTER.SURNAME AS TSURNAME\n"
                    + "FROM \n"
                    + "    ALDRIN.STOCK_IN \n"
                    + "INNER JOIN \n"
                    + "    ALDRIN.USER_ACCOUNT \n"
                    + "ON \n"
                    + "    ( \n"
                    + "        ALDRIN.STOCK_IN.USER_ID = ALDRIN.USER_ACCOUNT.ID) \n"
                    + "INNER JOIN \n"
                    + "    ALDRIN.TRANSPORTER \n"
                    + "ON \n"
                    + "    ( \n"
                    + "        ALDRIN.STOCK_IN.TRANSPORTER_ID = ALDRIN.TRANSPORTER.ID) ORDER BY STOCK_IN.CREATEDAT DESC ";
            getDBConn();
